...
const generateTableName = (data, nonce) => {
	debug('generating table name with nonce=%o', nonce);
	...
	debug('xor(data hash=%o, shuffle=%o)', dataHash, shuffle);
	const hash = Buffer.allocUnsafe(dataHash.length);
	for (let i = 0; i < hash.length; i++) {
		hash[i] = dataHash[i] ^ shuffle[i];
	}
	debug(hash);
...
// Deprecated
Module.createTable = ({ headers, data, types, nonce }, done) => {
	if (!pool) {
		throw new Error('module has no config');
	}
	const tableName = generateTableName(new Buffer(data), nonce);
